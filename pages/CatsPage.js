...
CatsPage.prototype.generate = async function() {
  ...
  this.blackCats.forEach((cat) => {
    let arrNames = ['Manel', 'Antonio', 'Eugènia', 'Eufràsia', 'Dídac', 'Antònia'];
    ...
    this.elements += `
      <article>
        ...
        <h3>${!cat.breeds.name ? arrNames[Math.floor(Math.random()*6)] : cat.breeds.name}</h3>
        <p>${!cat.breeds.description ? arrDescriptions[Math.floor(Math.random()*6)] : cat.breeds.description}</p>
